...
class RobotUiNode(Node):
    ...
    def _update_alerts(self):
        any_alert = (self._gps_state == "LOST")
...
class Dashboard(QWidget):
    def __init__(self, node=None, scale=1.0):
        ...
        self.exit_btn.setStyleSheet("QPushButton { background:transparent; border:1px solid #cc2f2f; color:#cc2f2f; border-radius:6px; padding:4px 8px; font-weight:bold; }")
        ...
        self.exit_btn.setFont(small_font)
        ...
        header.addWidget(self.exit_btn, alignment=Qt.AlignRight)
        ...
        # Row 0: HAND_HEART  | DANCE1               | SHAKE_HAND
        # Row 1: SIT         | STOP (spans 2 rows)  | STAND
        # Row 2: SPACE_STEP  |                      | DANCE2
        grid_buttons = {
            "HAND_HEART":  (0, 0, 1, 1),
            "DANCE1":      (0, 1, 1, 1),
            "SHAKE_HAND":  (0, 2, 1, 1),
            "SIT":         (1, 0, 1, 1),
            "STOP":        (1, 1, 2, 1),  # rowSpan=2
            "STAND":       (1, 2, 1, 1),
            "SPACE_STEP":  (2, 0, 1, 1),
            "DANCE2":      (2, 2, 1, 1),
        }
